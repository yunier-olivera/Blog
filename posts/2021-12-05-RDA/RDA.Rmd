...
El Análisis de Redundancia es una método para evaluar cuánta variación en una base de datos puede ser explicada por otra base de datos de variables explicativas. Es un análogo multivariado de la regresión.

# Análisis de Redundancia (RDA)

El análisis de redundancia puede verse como un análogo o una extensión de la regresión multiple linear, pero que incluye múltiples variables respuesta. Es un análisis de gradiente que evalúa las relaciones lineales entre variables respuestas que son "redundantes con" (esto significa: "explicadas por") un conjunto de variables explicativas [@buttigieg2014].

Algunos autores consideran al RDA como una extensión de los [Análisis de Componentes Principales (PCA)](https://yunierolivera.com/blog/pca/) que explícitamente modelan las variables respuesta como una función de las variables explicativas [@zuur_2007_analysing]. Usualmente los PCA solo incluyen variables respuesta, sin las variables explicativas, debido a que el PCA se enfoca en aquellas variables con correlaciones más altas (covarianza), que generalmente son las variables explicativas. Si las variables explicativas están altamente correlacionadas entre ellas, los gradientes principales tendrán información solo de dichas variables explicativas, haciendo el PCA una herramienta poco útil para evaluar el comportamientos de variables respuesta en estos casos. Aquí es donde entran los análisis de redundancia.
...
1.  Las variables explicativas cuantitativas son representadas por líneas y las variables explicativas cualitativas (nominales) son representadas por cuadrados (uno por cada nivel de esta variable nominal).
2.  Las variables respuesta se representan por lineas o etiquetas. Si hay muchas variables, es menos confuso el gráfico usando etiquetas.
...
Las opciones e interpretación son similares al del biplot de PCA. Primero decidimos el escalado: triplot de correlación o de distancia. Segundo, si usaremos una matriz de covarianza o una matriz de correlación para las variables respuesta. Lo nuevo con respecto a los biplots de los PCA, es el uso de líneas para representar las variables explicativas cuantitativas y los cuadrados para las cualitativas.
...
6.  Las observaciones se pueden proyectar perpendicularmente sobre las lineas de variables explicativas, e indican los valores de las variables explicativas en esos sitios.
...
Los resultados indican cuánto de las variables respuesta es explicado por las explicativas. De este porcentaje entonces dan que % es explicado en los dos primeros ejes.
